...
identify_missing_data <- function(portfolio_total){
  ...
  if(data_check(no_bbg_data)){
    ...
    no_bbg_data_all <- read_csv("data-raw/isins_missing_bbg_data.csv", col_types = "cc")  
    
    no_bbg_data_all <- bind_rows(no_bbg_data_all, no_bbg_data) %>% distinct()
    
    write_csv(no_bbg_data, "data-raw/isins_missing_bbg_data.csv")
  }
}
...
create_audit_file <- function(portfolio_total){
  ...
  audit_file <- portfolio_total %>% 
    select(all_of(grouping_variables), holding_id, isin, value_usd, company_name, security_bics_subgroup, asset_type,  has_revenue_data, valid_input, 
