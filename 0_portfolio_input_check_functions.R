...
map_security_sectors <- function(fin_data, sector_bridge) {
  ...
  fin_data_na <- fin_data %>%
    filter(is.na(sector)) %>%
    select(-c(sector, sector_boe, sector_ipr, subsector_ipr, sector_dnb, subsector_boe))

  fin_data <- fin_data %>% filter(!is.na(sector))

  fin_data_na <- fin_data_na %>%
    left_join(sector_bridge %>%
                filter(source == "ICB") %>%
                select(-source),
  ...
  fin_data <- fin_data %>%
    bind_rows(fin_data_na)

  fin_data <- fin_data %>%
    select(-security_mapped_sector, sector_boe, sector_ipr, subsector_ipr, sector_dnb, subsector_boe) %>%
...
map_comp_sectors <- function(comp_fin_data, sector_bridge) {
  ...
  comp_fin_data <- comp_fin_data %>% left_join(sector_bridge %>%
                                                 filter(source == "BICS") %>%
                                                 select(-source),
  ...
  comp_fin_data_na <- comp_fin_data_na %>%
    left_join(sector_bridge %>%
                filter(source == "ICB") %>%
                select(-source),
