...
outputs_folder <- file.path("new_st_inputs", "sensitivity_analysis")
...
params_to_analyze <- c(
  "discount_rate", "shock_year", "growth_rate",
...
compute_npv_and_pd_slopes_over_one_parameter_values <- function(tweaked_parameter_runs_data) {
  slopes_npv_over_param_tweaked <- tweaked_parameter_runs_data %>%
    ...
    dplyr::do(
      model_npv_baseline = lm(net_present_value_baseline ~ as.double(tweaked_param_value), data = .),
    ...
    dplyr::mutate(
      slope_npv_baseline = coef(model_npv_baseline)[2],
    ...
    dplyr::select(-c(
      "model_npv_baseline", "model_npv_shock",
  ...
  slopes_avg_pd_over_param_tweaked <- tweaked_parameter_runs_data %>%
    ...
    dplyr::summarise(
      mean_terms_pd_baseline = mean(pd_baseline),
    ...
    dplyr::do(
      model_mean_terms_pd_baseline = lm(mean_terms_pd_baseline ~ as.double(tweaked_param_value), data = .),
      model_mean_terms_pd_shock = lm(mean_terms_pd_shock ~ as.double(tweaked_param_value), data = .)
    ) %>%
    dplyr::mutate(
      slope_mean_terms_pd_baseline = coef(model_mean_terms_pd_baseline)[2],
...
draw_boxplots_foreach_param_value <- function(tweaked_parameter_runs_data, tweaked_parameter) {
  ...
  list(
    "boxplot_npv_baseline" = boxplot_npv_baseline,
...
draw_slopes_boxplots <- function(slopes_of_all_parameters) {
  ...
  list(
    "boxplot_slope_npv_baseline" = boxplot_slope_npv_baseline,
...
for (tweaked_parameter in params_to_analyze) {
  ...
  if (!is.null(tweaked_parameter_runs_data)) {
    ...
    #   ggsave(
    #     plot = tweaked_param_boxplots[[plot_name]],
    #     path = plots_folder,
...
ggsave(
  plot = plots_list$boxplot_slope_npv_baseline,
  path = plots_folder,
...
ggsave(
  plot = plots_list$boxplot_slope_npv_shock,
  path = plots_folder,
...
ggsave(
  plot = plots_list$boxplot_slope_mean_terms_pd_baseline,
  path = plots_folder,
...
ggsave(
  plot = plots_list$boxplot_slope_mean_terms_pd_shock,
  path = plots_folder,
