...
test_that("a non-empty fair_share_perc in the auto sector that is in the
  ...
  pre_fct_value_electric_2031 <- test_scenario_data %>% dplyr::filter(technology == "Electric", year == 2031) %>% dplyr::pull(fair_share_perc)
  ...
  post_fct_value_electric_2031 <- test_scenario_data %>% correct_automotive_scendata(interpolation_years = c(2031)) %>% dplyr::filter(technology == "Electric", year == 2031) %>% dplyr::pull(fair_share_perc)
...
test_that("no values are interpolated for years that are passed as
  ...
  pre_fct_value_coal_2031 <- test_scenario_data %>% dplyr::filter(technology == "Coal", year == 2031) %>% dplyr::pull(fair_share_perc)
  pre_fct_is.na_coal_2031 <- is.na(test_scenario_data %>% dplyr::filter(technology == "Coal", year == 2031) %>% dplyr::pull(fair_share_perc)) == TRUE
  post_fct_value_coal_2031 <- test_scenario_data %>% correct_automotive_scendata(interpolation_years = c(2031)) %>% dplyr::filter(technology == "Coal", year == 2031) %>% dplyr::pull(fair_share_perc)
