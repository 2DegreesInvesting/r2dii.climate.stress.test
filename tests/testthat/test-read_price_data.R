...
test_that("with valid arguments set, read_price_data() return data.frame", {
  ...
  test_data_prices %>% readr::write_csv(file.path(tempdir(), "prices_data_input.csv"))

  test_input_path <- file.path(tempdir(), "prices_data_input.csv")

  test_data <- read_price_data(path = test_input_path)
