...
The goal of r2dii.climate.stress.test is to calculate climate change related market and credit risk for listed equity, corporate bonds and corporate loans.
In order to analyse sensitivities the several model parameters can be altered.
Note: Application of the code requires availability of custom data. In order to participate in a climate stress testing project with 2 degrees investing initiative please contact jacob@2degrees-investing.org
...
run_stress_test(asset_type_arg = "loans", shock_year = 2025:2035)

## run stress testing for asset of type equity using various risk free rates to analyse sensitivities
run_stress_test(asset_type_arg = "equity", risk_free_rate = c(0.01, 0.025))
```

## Details
* Prepare input data
Stress tests for corporate loans require an additional step for input data preparation for initial use on a loanbook.
```{r example}
run_prep_calculation_loans()

```

* Lookup valid ranges of model parameters
