...
calculated for). Compare level `Global` for the maximum number of supported sectors.
All supported scenario geographies are the following:

`Global, AdvancedEconomies, Africa, AsiaPacific,` 
...
You can use the following command see the complete sector and scenario geography coverage
  
```{r}
print(scenario_geography_x_ald_sector, n = 100)
```
