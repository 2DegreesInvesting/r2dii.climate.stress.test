...
test for loans, using the function `run_trisk` to calculate the credit risk of a
loan book.
...
to pass exactly one value per input argument to the function `run_trisk()`.
...
`run_trisk()` can be found in column `default`. The user may change any detail
