...
parameters or as a sensitivity analysis for a set of parameters that iterates
over one of the input arguments. We will look at both cases.
...
The only arguments that do not have a default
value and must be set by the user are `asset_type` and the file paths. In this
...
The user may change any detail arguments by explicitly passing it to the
function. Note that there are allowed ranges for each of the inputs. The passed
argument must be within these ranges.

You can look up allowed values of input arguments by running:
```{r}
...
to approximate further cash flows beyond the time frame of the model.
...
### Output files
The output files are written into a subdirectory of the path indicated as
`output_path` in the function call. The name of the subdirectory is the
timestamp of the analysis. It will be generated automatically.
...
**NOTE:** The suffix `_standard` indicates that the stress test was run for a
