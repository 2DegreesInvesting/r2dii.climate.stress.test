...
We find this is the easiest and most user friendly way to obtain results, but we
acknowledge that more advanced R users may find this to depart from the
conventions of other packages. Below we highlight, how to return the results as
objects within the R session.

### Input arguments
Specifications on the detail arguments are summarised in the following overview.
...
returned as objects within the running session. The default is FALSE, which
writes the outputs to the target directory and does not return them as objects.
...
If the user wants to view the results directly in RStudio, they can set the
argument to `TRUE` which will return them as data frames in the R session. This
...
user must still provide an output path though, as every calculation will write
a log file.
