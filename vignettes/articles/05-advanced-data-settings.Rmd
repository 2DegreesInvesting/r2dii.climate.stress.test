---
title: "05 - Improve stress test by providing further project specific data"
---
...
fin_companies <- data %>%
  ...
  dplyr::mutate(
    ...
    volatility = NA)
readr::write_csv(fin_companies, file.path("/path/to/agnostic/data", "prewrangled_financial_data_stress_test.csv")) # NOTE that this will overwrite financial data currently stored at the location if available
```
...
You can now run the stress test as described in vignette ["Run stress test"](https://2degreesinvesting.github.io/r2dii.climate.stress.test/articles/articles/04-run-stress-test.html).
