...
```{r, eval = FALSE}
Sys.setenv(
MLFLOW_PYTHON_BIN = "/Users/username/opt/miniconda3/envs/mlflow_env/bin/python",
...
It is possible to stop this function while it runs, and restarting is with the same parameters names. It is possible to change the values of each parameter, but not changing the names / number of parameters tested. When changing the names of parameters, 

It is recommended to change the value of experiment_name when changing the names of parameters present in the params_grid variable, because changing those names will generate an error when fetching and filtering previous runs parameters.

The results are then available to browse in the MLFlow UI, or to be analyzed by searching
runs, or downloading artifacts, using the following workflow.

# Searching runs and extracting their outputs

#### Using native functions from the MLFlow package
...
# unzip in-memory and read the desired file, crispy_output.csv or company_trajectories.csv
f_conn <- unz(file.path(artifacts_path, "artifacts.zip"), "crispy_output.csv")
artifact <- readr::read_csv(f_conn, show_col_types = FALSE)
```

#### Aggregating all TRISK outputs where a given parameter is tweaked

```{r, eval=FALSE}
  tweaked_parameter_runs_data <- gather_trisk_outputs_for_tweaked_parameter(
    tracking_uri="http://localhost:5000",
    experiment_name="EXPERIMENT_NAME",
    tweaked_parameter="shock_year",
